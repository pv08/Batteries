...
class Optmizer:
    def __init__(self, model_name, n_agents, n_communities, day, hour, optimization_results,
                 agents_results, communities_results, battery_allocation, battery_size, agents_data, profiles, agents_df,
                 cost_df, data, case):
        ...
        self.model = ConcreteModel(model_name)


        self.agent_range_set = range(n_agents)
        self.community_set = range(n_communities - 1)
        self.community_range_set = range(n_communities**2)
    ...
    def obj_fn_quadratic(self, model):
        ...
        for j in range(0, self.n_communities ** 2):
    ...
    def defineInequalityCons(self, day, hour):
        ...
        bat_dict = self.data[f'BatteryList_Caso0{self.case}']
    ...
    def defineEqualityCons(self, day, hour):
        ...
        for j, row in self.agents_df.iterrows():
            ...
            if not self.battery_allocation:
                ...
                if row['type'] == 'EXT_PRODUCER':  # external grid producer
                    ...
                    self.model.Equality_Cons.add(self.model.Soc_n[j] == 0)
                    self.model.Equality_Cons.add(self.model.Alpha_n[j] == 0)
        community_connex_var = np.reshape(self.community_range_set, (self.n_communities, self.n_communities))
        for j in range(1, self.n_communities + 1):
            ...
            for i in range(0, self.n_communities):
        ...
        for j in range(0, self.n_communities):
            for i in range(0, self.n_communities):
            ...
            if (day == 0 and hour == 0):  ### DEFINES THE STARTING STATE OF CHARGE AS 0.5
                ...
                for i in range(0, self.n_agents):
                    if (i in idx):
                        ...
                        j = j + 1
            elif (day == self.day - 1 and hour == self.hour - 1):  ### DEFINES THE FINAL STATE OF CHARGE AS 0.5
                idx = np.where(self.agents_df['TYPE'][:] == 'BATTERY')[0]  # batteries locations -> batteries as agents
                j = 0
                for i in range(0, self.n_agents):
                    if (i in idx):
    ...
    def minimize(self, hour, day, cost_df):
        self.cost_df = cost_df
        self.case_dict = {
            1: return_matrix_case1to5(cost_df=self.cost_df, n_communities=self.n_communities),
            2: return_matrix_case1to5(cost_df=self.cost_df, n_communities=self.n_communities),
            3: return_matrix_case1to5(cost_df=self.cost_df, n_communities=self.n_communities),
            4: return_matrix_case1to5(cost_df=self.cost_df, n_communities=self.n_communities),
            5: return_matrix_case1to5(cost_df=self.cost_df, n_communities=self.n_communities),
            6: return_matrix_case6(cost_df=self.cost_df, n_communities=self.n_communities),
            7: return_matrix_case7(cost_df=self.cost_df, n_communities=self.n_communities)
        }

        mat_bilateral, import_cost, export_cost = self.case_dict[self.case]

        self.imp_cost = np.reshape(np.multiply(-import_cost, mat_bilateral), [1, self.n_communities ** 2])
        self.exp_cost = np.reshape(np.multiply(export_cost, mat_bilateral), [1, self.n_communities ** 2])
...
class OptimizationData:
    def __init__(self, args, bus_list, battery_allocation, bat_bus_location: list = None):
        self.args = args
        self.battery_allocation = battery_allocation
        self.bat_bus_location = bat_bus_location
        self.dssConfig = DSSConfig(root_dir=args.root_dir, battery_size=args.battery_size, study_case=args.case,
                                   profiles=args.profiles, battery_allocation=battery_allocation,
                                   bat_bus_location=bat_bus_location)
    ...
    def optNoStorage(self, day_range, hour_range, n_agents, n_communities):
        agents_results = [[{hour: {} for hour in range(hour_range)} for _ in range(day_range)] for _ in range(n_agents)]
        communities_results = [[{hour: {} for hour in range(hour_range)} for _ in range(day_range)] for _ in range(n_communities)]
        optimization_results = [{hour: {} for hour in range(hour_range)} for _ in range(day_range)]
        opt_logger = [{hour: {} for hour in range(hour_range)} for _ in range(day_range)]
        # day: list, hour: dict, bus: list
        community_bus_results = []
        # day: list, hour: dict, line: list
        community_lines_results = []
        for day in range(day_range):
            for hour in range(hour_range):
                #Pega os dados em função da hora
                data = PreProcessingData(self.args, battery_allocation=self.battery_allocation, bat_loc=self.bat_bus_location)
                costs = data.costInfo(hour=hour)
                # Realiza a otimização com os dados
                opt = Optmizer(model_name=self.args.model_name, n_agents=n_agents, n_communities=n_communities, day=day,
                               hour=hour, optimization_results=optimization_results, agents_results=agents_results,
                               communities_results=communities_results, battery_allocation=self.battery_allocation,
                               battery_size=self.args.battery_size, agents_data=data.agents_data, profiles=data.profiles,
                               agents_df=data.agents_df, cost_df=costs, data=data.data, case=self.args.case)

                model, results, inner_community_balance, outer_community_balance = opt.minimize(hour=hour, day=day, cost_df=costs)
                # Salva os resultados do otimizador
                opt_logger[day][hour] = {'fob': value(model.obj),
                ...
                #Gera os reports
                report = Report(n_communities=n_communities, agents_df=data.agents_df, day=day, hour=hour,
                                battery_allocation=self.battery_allocation, n_agents=n_agents,
                ...
                report.set_report(model=model, results=results,
                                  inner_community_balance=inner_community_balance,
                                  outer_community_balance=outer_community_balance)

                #realiza o update das curvas para a próxima iteração

                data.updateProfiles(hour=hour, agt_results=report.agt_results)

                #faz o fluxo de potência
                powerflow = Powerflow(battery_size=self.args.battery_size,
                                      battery_allocation=self.battery_allocation,
                                      case=self.args.case,
                                      hour=hour,
                                      day=day,
                                      bat_list=None,
                                      lv_bus_list=data.data['BusListLV']['NAME'],
                                      lvbus_basekv_list=data.data['BusListLV']['BASE_KV'])
                bus_results, line_results = powerflow.calcDSSPowerflow(master_file=self.dssConfig.masterfile,
                                           community_buses=data.community_buses,
                                           community_lines=data.community_lines)

                community_bus_results.append({hour: bus_results})
                community_lines_results.append({hour: line_results})
                os.chdir(self.args.root_dir)
                del powerflow, opt, data
        return {'agents_results': agents_results,
                'communities_results': communities_results,
                'optimization_results': optimization_results,
                'opt_logger': opt_logger,
                'community_bus_results': community_bus_results,
                'community_lines_results': community_lines_results
                }
    ...
    def optWithStorage(self, day_range, hour_range, n_agents, n_communities, bat_bus_location):
        agents_results = [[{hour: {} for hour in range(hour_range)} for _ in range(day_range)] for _ in range(n_agents)]
        communities_results = [[{hour: {} for hour in range(hour_range)} for _ in range(day_range)] for _ in range(n_communities)]
        optimization_results = [{hour: {} for hour in range(hour_range)} for _ in range(day_range)]
        opt_logger = [{hour: {} for hour in range(hour_range)} for _ in range(day_range)]
        ...
        for day in range(self.args.day):
            for hour in range(self.args.hour):
                # Pega os dados em função da hora
                data = PreProcessingData(self.args, battery_allocation=self.battery_allocation,
                                         bat_loc=self.bat_bus_location)
                costs = data.costInfo(hour=hour)
                # Realiza a otimização com os dados
                opt = Optmizer(model_name=self.args.model_name, n_agents=n_agents, n_communities=n_communities, day=day,
                               hour=hour, optimization_results=optimization_results, agents_results=agents_results,
                               communities_results=communities_results, battery_allocation=self.battery_allocation,
                               battery_size=self.args.battery_size, agents_data=data.agents_data,
                               profiles=data.profiles,
                               agents_df=data.agents_df, cost_df=costs, data=data.data, case=self.args.case)

                model, results, inner_community_balance, outer_community_balance = opt.minimize(hour=hour, day=day,
                                                                                                cost_df=costs)
                # Salva os resultados do otimizador
                opt_logger[day][hour] = {'fob': value(model.obj),
                                         'solver_status': results.solver.status,
